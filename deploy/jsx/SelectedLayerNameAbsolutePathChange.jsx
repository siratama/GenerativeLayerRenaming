...
GenerativeLayerRenaming.prototype = {
	...
	,setAbsolutePathInSelectedLayer: function() {
	...
	,removeAbsolutePath: function() {
		this.activeDocument = this.application.activeDocument;
		var layerStructure = new jsx_parser_layer_LayerStructure(this.activeDocument,this.activeDocument.layers,[]);
		layerStructure.parse();
		layerStructure.removeAbsolutePathInLayer();
	}
...
jsx__$GenerativeLayerRenaming_GenerativeLayerRenamingJSXRunner.setAbsolutePathInSelectedLayer = function() {
	var frameAnimationExport = new GenerativeLayerRenaming();
	...
	case 1:
		...
		break;
	case 0:
		frameAnimationExport.setAbsolutePathInSelectedLayer();
		break;
	}
};
jsx__$GenerativeLayerRenaming_GenerativeLayerRenamingJSXRunner.removeAbsolutePath = function() {
	var frameAnimationExport = new GenerativeLayerRenaming();
	var errorEvent = haxe_Unserializer.run(frameAnimationExport.getInitialErrorEvent());
	switch(errorEvent[1]) {
	case 1:
		...
		break;
	case 0:
		frameAnimationExport.removeAbsolutePath();
		break;
	}
};
...
jsx_parser_layer_LayerStructure.prototype = {
	...
	,removeAbsolutePathInLayer: function() {
		var _g = 0;
		var _g1 = this.layerPropertySet;
		while(_g < _g1.length) {
			var layerProperty = _g1[_g];
			++_g;
			var layer = layerProperty.layer;
			var layerName = layerProperty.layer.name;
			var index = layerName.indexOf(".png");
			if(index == -1) continue;
